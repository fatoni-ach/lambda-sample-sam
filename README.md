...
```bash
sample-sam-toni$ sam build --use-container
```
...
```bash
sample-sam-toni$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sample-sam-toni$ sam local start-api
sample-sam-toni$ curl http://localhost:3000/
```
...
```bash
sample-sam-toni$ sam logs -n HelloWorldFunction --stack-name sample-sam-toni --tail
```
...
```bash
sample-sam-toni$ pip install -r tests/requirements.txt --user
# unit test
sample-sam-toni$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sample-sam-toni$ AWS_SAM_STACK_NAME="sample-sam-toni" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "sample-sam-toni"
```
...
or 
```shell
# deploy with mode watch
sam sync --region ap-southeast-1 --stack-name sample-sam-toni --watch
```